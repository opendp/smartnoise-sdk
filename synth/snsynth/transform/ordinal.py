...
class OrdinalTransformer(CachingColumnTransformer):
    ...
    def cardinality(self):
        return [self.fit_upper - self.fit_lower + 1]
    ...
    def _fit_finish(self):
        if self.epsilon is not None and self.epsilon > 0.0 and (self.lower is None or self.upper is None):
            ...
            if self.fit_lower is None or self.fit_upper is None:
                raise ValueError("OrdinalTransformer could not find bounds.")
        elif self.lower is None or self.upper is None:
            raise ValueError("OrdinalTransformer requires either epsilon or min and max.")
        else:
    ...
    def _transform(self, val):
        if not self.fit_complete:
            raise ValueError("OrdinalTransformer has not been fit yet.")
