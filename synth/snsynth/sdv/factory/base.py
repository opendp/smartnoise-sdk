...
from snsynth.sdv.errors import NotFittedError
from snsynth.sdv.metadata import Metadata, utils

LOGGER = logging.getLogger(__name__)


class BaseModel:
    """Base class for all the factory models.

    The ``BaseModel`` class defines the common API that all the
    factory models need to implement, as well as common functionality.
    ...
    def _fit(self, tables=None):
        """Fit this factory model instance to the dataset data.
    ...
    def sample(self, table_name=None, num_rows=None, reset_primary_keys=False): # sample_children=True,
        """Generate synthetic data for one table or the entire dataset.
