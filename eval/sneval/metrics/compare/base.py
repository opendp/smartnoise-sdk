...
class CompareMetric(Metric):
    ...
    def param_names(self):
        return super().param_names() + ["categorical_columns", "measure_columns"]
    def validate(self, original : Dataset, synthetic : Dataset):
        ...
        if ( not original.is_aggregated and not set(self.measure_columns).issubset(set(original.measure_columns)) ) or \
            ( original.is_aggregated and not set(self.measure_columns).issubset(set(original.sum_columns)) ):
