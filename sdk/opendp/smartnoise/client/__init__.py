...
class ExecutionClient(object):
    def submit(self, params, uri):
        client = _get_client()
        details = ProjectRunDetails(params=json.dumps(params), project_uri=uri)
...
class DatasetClient(object):
    ...
    datasets using the opendp-smartnoise service
    """
...
def get_dataset_client():
    client_overrides = [
        entrypoint
        for entrypoint in pkg_resources.iter_entry_points("opendp_smartnoise_dataset_client")
    ]
