...
class MWEMSynthesizer(SDGYMBaseSynthesizer):
    ...
    def mwem(self):
        """
        Runner for the mwem algorithm. Initializes the synthetic histogram, and updates it
        for self.iterations using the exponential mechanism and multiplicative weights.
        Draws from the initialized query store for measurements.
    ...
    def _initialize_a_values(self, histogram, dimensions):
        """
        ...
        :return: New histogram, uniformly distributed according to reference histogram.
        :rtype: np.ndarray
        """
        ...
        a_values = np.zeros_like(histogram)
        a_values += value
        return a_values
    ...
    def _histogram_from_data_attributes(self, data, splits=[]):
        ...
        :type data: np.ndarray
        :return: Histogram over given data, dimensions, bins created (output of np.histogramdd)
    ...
    def _exponential_mechanism(self, hist, a_values, q_values, eps):
        """
        Refer to paper for in depth description of Exponential Mechanism.
        Parametrized with epsilon value epsilon/(2 * iterations).

        :param hist: Basis histogram
        :type hist: np.ndarray
        :param a_values: Synthetic histogram
        :type a_values: np.ndarray
        :param q_values: Queries to draw from
        :type q_values: list
        :param eps: Budget
        :type eps: float
        :return: # of errors
        ...
        """
        errors = [
            abs(self._evaluate(q_values[i], hist) - self._evaluate(q_values[i], a_values)) * (eps / 2.0)
    ...
    def _multiplicative_weights(self, a_values, q_values, m, hist, iterate):
        """
        Multiplicative weights update algorithm, used to boost the synthetic data
        accuracy given measurements m. Run for iterate times
        ...
        :rtype: np.ndarray
        """
        sum_a_values = np.sum(a_values)
        for _ in range(iterate):
            for qi in m:
                error = m[qi] - self._evaluate(q_values[qi], a_values)
                # Perform the weights update
                query_update = self._binary_replace_in_place_slice(
                                   np.zeros_like(a_values.copy()), q_values[qi])

                # Apply the update
                a_values_multiplier = np.exp(query_update * error / (2.0 * sum_a_values))
                a_values_multiplier[a_values_multiplier == 0.0] = 1.0
                a_values = a_values * a_values_multiplier
                # Normalize again
                count_a_values = np.sum(a_values)
                a_values = a_values * (sum_a_values / count_a_values)
        return a_values
    ...
    def _compose_arbitrary_slices(self, num_s, dimensions):
        ...
        random slice objects, given the dimensions. These are our linear queries.
        ...
        :return: Collection of random np.s_ (linear queries) for a dataset with dimensions
        :rtype: list
        """
        slices_list = []
    ...
    def _binary_replace_in_place_slice(self, data, a_slice):
        """
        We want to create a binary copy of the data, so that we can easily perform
        our error multiplication in MW. Convenience function.

        :param data: Data
        :type data: np.ndarray
        :param a_slice: Slice
        :type a_slice: np.s_
        :return: Return data, where the range specified by a_slice is all 1s.
        :rtype: np.ndarray
        """
        view = data.copy()
    ...
    def _reorder(self, splits):
        ...
        Example:
            original = [[1, 2, 3, 4, 5, 6], [6, 7, 8, 9, 10, 11]]

            splits = [[1, 3, 4],[0, 2, 5]]

            mod_data = [[2, 4, 5, 1, 3, 6] [7, 9, 10,  6,  8, 11]]
            reorder = [3, 0, 4, 1, 2, 5]
    ...
    def _generate_splits(self, n_dim, factor):
        ...
        into even splits to speed up MWEM. Last split will contain leftovers <= sizeof(factor)
.
        ...
        s2 = [np.array(split) for split in s1]
        return np.array(s2)
