...
class DPVerification:
    ...
    def __init__(self, epsilon=1.0, dataset_size=10000, csv_path="."):
        ...
        self.d1, self.d2, self.d1_metadata, self.d2_metadata = None, None, None, None
    ...
    def dp_powerset_test(self, query_str, table_name, debug=False, plot=True, bound=True, exact=False, repeat_count=10000, confidence=0.95, test_cases=5):
        """
        ...
        for sample in halton_samples:
            df = ex.create_small_dataset(sample)
            ex.generate_powerset(df)
            print("Test case: ", list(sample))
            for filename in ex.visited:
                print("Testing: ", filename)
                d1_query = query_str.replace(table_name, "d1_" + filename + "." + "d1_" + filename)
                d2_query = query_str.replace(table_name, "d2_" + filename + "." + "d2_" + filename)
                [self.d1, self.d2, self.d1_metadata, self.d2_metadata] = ex.neighbor_pair[filename]
