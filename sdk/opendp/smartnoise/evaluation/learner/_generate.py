...
class Token(object):
    def __init__(self, token, children):
        ...
        self.children = None if children is None else [c for c in children if c.token != '' and c.token != 'null']
    ...
    def __str__(self):
        if self.literal:
            ...
            return "".join(s for s in [str(c) for c in self.children] if s != "").replace(" ", "").replace(",","")
        else:
            return " ".join(s for s in [str(c) for c in self.children] if s != "")
...
class Grammar(object):
    ...
    def is_builtin(token):
        return any([token.startswith(q) for q in ['rand_int(','rand_float(','rand_string(', 'collapse(']]) or token == 'null'
    ...
    def generate_builtin(self, token):
        ...
        if bare == "collapse":
            ...
            val = Grammar.val_between(token, '(', ')')
            return Grammar.make_literal('"' + "".join([np.random.choice(list(string.ascii_letters)) for i in range(int(val))]) + '"')
        else:
    ...
    def load(self, rules):
        ...
        for line in rules:
            ...
            for quant in [t for t in chain.from_iterable(tokens) if self.is_quantifier(t) and (t not in grammar)]:
