...
class TestLaplace:
    ...
    def test_bounds1c_lap(self):
        ...
        g = Laplace(1.0, confidence_widths=[0.1]) # epsilon of 1.0
        lower, upper = g.bounds(False)[0] # analytic bounds
        lower2, upper2 = g.bounds(True)[0]  # bootstrap bounds
        assert(lower <= upper)
        assert(lower2 <= upper2)
    def test_bounds2_lap(self):
        ...
        g = Laplace(4.0, confidence_widths=[0.95, 0.97]) # epsilon of 4.0, tighter bounds
    ...
    def test_intervals_lap(self):
        l = Laplace(4.0, confidence_widths=[0.95, 0.5])
        vals = [100, 3333, 99999]
