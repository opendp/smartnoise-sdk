...
class TestWhitenoiseCore:
    def test_dp_whitenoise_core_mean_pums(self):
        ...
        dp_res, bias_res = dv.whitenoise_core_test(test_csv_path, test_csv_names, op.dp_mean, 'race', "FLOAT", epsilon=.65, actual = actual_mean, data_min=0., data_max=100., data_n=1000)
    ...
    def test_dp_whitenoise_core_var_pums(self):
        logging.getLogger().setLevel(logging.DEBUG)
        dp_res, bias_res = dv.whitenoise_core_test(test_csv_path, test_csv_names, op.dp_variance, 'educ', "FLOAT", epsilon=.15, actual = actual_var, data_min=0., data_max=12., data_n=1000)
    ...
    def test_dp_whitenoise_core_moment_pums(self):
        logging.getLogger().setLevel(logging.DEBUG)
        dp_res, bias_res = dv.whitenoise_core_test(test_csv_path, test_csv_names, op.dp_moment_raw, 'race', "FLOAT", epsilon=.15, actual = actual_moment, data_min=0., data_max=100., data_n=1000, order = 3)
    ...
    def test_dp_whitenoise_core_covariance_pums(self):
        logging.getLogger().setLevel(logging.DEBUG)
        dp_res, bias_res = dv.whitenoise_core_test(test_csv_path, test_csv_names, op.dp_covariance, 'age', 'married', "FLOAT", actual = actual_covariance, epsilon=.15, left_n=1000, right_n=1000,left_min=0.,left_max=1.,right_min=0.,right_max=1.)
