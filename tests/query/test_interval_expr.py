...
class TestQuery:
    def test_group_by_noisy_typed_order_inter(self):
        ...
        assert(len(rs.release['c'].intervals[0.95]) == 2)
        print(rs.release['c'].intervals[0.95])
        assert(all(ival.low < ival.high for ival in rs.release['c'].intervals[0.95]))
        assert(all(ival.low < ival.high for ival in rs.release['c'].intervals[0.985]))
        assert(all(outer.low < inner.low for inner, outer in zip(rs.release['c'].intervals[0.95], rs.release['c'].intervals[0.985])))
        assert(all(outer.high > inner.high for inner, outer in zip(rs.release['c'].intervals[0.95], rs.release['c'].intervals[0.985])))
    def test_group_by_noisy_typed_order_inter_constant(self):
        ...
        assert(len(rs.release['c'].intervals[0.95]) == 2)
        assert(len(rs2.release['c'].intervals[0.95]) == 2)
        assert(all(a.low < b.low for a, b in zip(rs.release['c'].intervals[0.95], rs2.release['c'].intervals[0.95])))
        assert(all(a.low < b.low for a, b in zip(rs.release['c'].intervals[0.985], rs2.release['c'].intervals[0.985])))
