...
class BigQueryFactory(DbFactory):
    ...
    @staticmethod
    def _load_service_account_info(credentials_value):
        if os.path.isfile(credentials_value):
            with open(credentials_value, "r", encoding="utf-8") as credentials_file:
                return json.load(credentials_file)

        return json.loads(credentials_value, strict=False)
    ...
    def connect(self, dataset):
        ...
        try:
            ...
            if "GOOGLE_APPLICATION_CREDENTIALS" in os.environ:
                ...
                info = self._load_service_account_info(creds)
        ...
        except (ImportError, OSError, TypeError, ValueError) as err:
            print(
                f"Unable to connect to BigQuery dataset {dataset}.  "
                f"Ensure connection info is correct and gcp library is installed. {err}"
            )
